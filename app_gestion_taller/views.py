...
@csrf_exempt
def buscar_servicio_de_coche(request, coche_id):
    try:
...
def registrar_servicio_coche(request):  
    if request.method == 'POST':
        try:
            ...
            CocheServicio.objects.create(coche=coche, servicio=servicio)
            return JsonResponse({"mensaje": "Servicio asignado al coche con éxito"})
        except Coche.DoesNotExist:
            return JsonResponse({"error": "Coche no encontrado"}, status=404)
...
def nuevo_coche(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('detalle_cliente', cliente_id=form.instance.cliente.id)
    else:
        form = CocheForm()
...
def eliminar_cliente(request, cliente_id):
    ...
    return redirect('lista_clientes')
    #return render(request, 'eliminar.html', { 'form': cliente, 'titulo': 'Eliminar Cliente' })